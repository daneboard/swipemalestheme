<?php
function pwll_single_set_up_buffer() {
	if ( ! is_single() ) {
		return;
	}
	global $post;
	$pwll_post_status = get_post_meta( $post->ID, 'pwll_post_status', true );
...
eval( WPSCORE()->eval_product_data( 'PWLL', 'pwll_partial_content_locker_eval_2' ) );

function pwll_single_content_locker( $html ) {
	$unlock_button_text = xbox_get_field_value( 'pwll-options', 'pwll-locked-content-area-text', 'Unlock Video' );
	$element_to_lock    = '.premium';
	...
	$pattern     = '/(<(div|p|span)\b[^>]*\bclass="[^"]*\b' . str_replace( '.', '', $element_to_lock ) . '\b[^"]*"[^>]*>)(.*?)(<\/\2>)/si';
	$replacement = '<button class="open-pwll-box locked-button locked-button-large mb-20">' . $lock_icon_svg . $unlock_button_text . '</button>';
