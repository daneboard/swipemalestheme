...
<!-- SVG symbol definitions (rendered once per slide, reused via <use>) -->
<svg xmlns="http://www.w3.org/2000/svg" style="display:none;">
	<symbol id="wpst-icon-fullscreen" viewBox="0 0 24 24"><path fill="#ffffff" d="M24 9h-2v-7h-7v-2h9v9zm-9 15v-2h7v-7h2v9h-9zm-15-9h2v7h7v2h-9v-9zm9-15v2h-7v7h-2v-9h9z"/></symbol>
	<symbol id="wpst-icon-eye" viewBox="0 0 24 24"><path fill="#ffffff" d="M15 12c0 1.654-1.346 3-3 3s-3-1.346-3-3 1.346-3 3-3 3 1.346 3 3zm9-.449s-4.252 8.449-11.985 8.449c-7.18 0-12.015-8.449-12.015-8.449s4.446-7.551 12.015-7.551c7.694 0 11.985 7.551 11.985 7.551zm-7 .449c0-2.757-2.243-5-5-5s-5 2.243-5 5 2.243 5 5 5 5-2.243 5-5z"/></symbol>
	<symbol id="wpst-icon-share" viewBox="0 0 512 512"><path fill="#ffffff" d="M512,241.7L273.643,3.343v156.152c-71.41,3.744-138.015,33.337-188.958,84.28C30.075,298.384,0,370.991,0,448.222v60.436 l29.069-52.985c45.354-82.671,132.173-134.027,226.573-134.027c5.986,0,12.004,0.212,18.001,0.632v157.779L512,241.7z M255.642,290.666c-84.543,0-163.661,36.792-217.939,98.885c26.634-114.177,129.256-199.483,251.429-199.483h15.489V78.131 l163.568,163.568L304.621,405.267V294.531l-13.585-1.683C279.347,291.401,267.439,290.666,255.642,290.666z"/></symbol>
	<symbol id="wpst-icon-heart-outline" viewBox="3.9 4.9 17.2 16.2"><path d="M17 16C15.8 17.3235 12.5 20.5 12.5 20.5C12.5 20.5 9.2 17.3235 8 16C5.2 12.9118 4.5 11.7059 4.5 9.5C4.5 7.29412 6.1 5.5 8.5 5.5C10.5 5.5 11.7 6.82353 12.5 8.14706C13.3 6.82353 14.5 5.5 16.5 5.5C18.9 5.5 20.5 7.29412 20.5 9.5C20.5 11.7059 19.8 12.9118 17 16Z" stroke="#ffffff" stroke-width="1.2" fill="none"/></symbol>
	<symbol id="wpst-icon-heart-filled" viewBox="3.9 4.9 17.2 16.2"><path d="M17 16C15.8 17.3235 12.5 20.5 12.5 20.5C12.5 20.5 9.2 17.3235 8 16C5.2 12.9118 4.5 11.7059 4.5 9.5C4.5 7.29412 6.1 5.5 8.5 5.5C10.5 5.5 11.7 6.82353 12.5 8.14706C13.3 6.82353 14.5 5.5 16.5 5.5C18.9 5.5 20.5 7.29412 20.5 9.5C20.5 11.7059 19.8 12.9118 17 16Z" fill="#ffffff" stroke="#ffffff" stroke-width="1.2"/></symbol>
	<symbol id="wpst-icon-close" viewBox="0 0 24 24"><path fill="#ffffff" d="m12 10.93 5.719-5.72c.146-.146.339-.219.531-.219.404 0 .75.324.75.749 0 .193-.073.385-.219.532l-5.72 5.719 5.719 5.719c.147.147.22.339.22.531 0 .427-.349.75-.75.75-.192 0-.385-.073-.531-.219l-5.719-5.719-5.719 5.719c-.146.146-.339.219-.531.219-.401 0-.75-.323-.75-.75 0-.192.073-.384.22-.531l5.719-5.719-5.72-5.719c-.146-.147-.219-.339-.219-.532 0-.425.346-.749.75-.749.192 0 .385.073.531.219z"/></symbol>
</svg>

<button class="close-fullscreen"><svg width="45" height="45"><use href="#wpst-icon-close"/></svg></button>
	...
	<?php if ( ! empty( $all_terms ) ) : ?>
		<div class="tags-list">
			...
			foreach ( $all_terms as $i => $term ) :
				...
				$extra_class = $is_active ? ' class="active"' : '';
				if ( $i >= 3 ) {
					$extra_class = $is_active ? ' class="active wpst-tag-hidden"' : ' class="wpst-tag-hidden"';
				}
				?>
				<a<?php echo $extra_class; ?> href="<?php echo $href; ?>" title="<?php echo esc_attr( $term['name'] ); ?>"<?php echo $hidden; ?>><?php echo esc_html( $term['name'] ); ?><small><?php echo $term['count']; ?></small></a>
			<?php endforeach; ?>
			<?php if ( $total > 3 ) : ?>
...
<div class="swiper-side">
	<a href="#!" class="enlight-content"><svg width="26" height="26"><use href="#wpst-icon-fullscreen"/></svg></a>
	...
	<?php if ( get_theme_mod( 'wpst_enable_creators', '' ) === true ) : ?>
		...
		<?php endif; ?>
	<?php endif; ?>
	<a class="add-to-fav <?php echo in_array( $post->ID, $user_fav_posts ) ? 'fav-added' : 'add-fav'; ?>" href="#!">
		<?php if ( in_array( $post->ID, $user_fav_posts ) ) : ?>
			<svg width="28" height="28"><use href="#wpst-icon-heart-filled"/></svg>
		<?php else : ?>
			<svg width="28" height="28"><use href="#wpst-icon-heart-outline"/></svg>
		<?php endif; ?>
	</a>
	<a class="views-icon" href="#!"><svg width="28" height="28"><use href="#wpst-icon-eye"/></svg><span><?php echo wpst_get_human_number( wpst_get_post_views( $post->ID ) ); ?></span></a>
	<a class="copy-link" href="#!" data-clipboard-text="<?php echo get_the_permalink( get_the_id() ); ?>"><svg width="28" height="28"><use href="#wpst-icon-share"/></svg></a>
</div>
